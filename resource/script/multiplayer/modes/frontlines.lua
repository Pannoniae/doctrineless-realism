...
local isFLAttacker = false
local isFLDefender = false
...
-- Human tags
	...
	return GetRandomItem(unitsToSpawn, function(t)
		...
		if UnitType("Cannon") then
			...
			if isFLAttacker then
				priorityMultiplier = priorityMultiplier * 0.10
			end
			if isFLDefender then
				priorityMultiplier = priorityMultiplier * 1.40 
			end
		end
		...
		if not UnitType("Cannon") or not UnitType("Squad") then
			...
			if isFLAttacker then
				priorityMultiplier = priorityMultiplier * 1.30
			end
			if isFLDefender then
