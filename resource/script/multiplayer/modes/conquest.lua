...
local function setVarsInMissionScript(bot_army)
	if teamSize > 1 then
...
function OnGameStart()
	...
	bot_army_local = BotApi.Instance.army
	math.randomseed(os.time()*BotApi.Instance.hostId)
	math.random() math.random() math.random() math.random()
	
	local rnd = math.random()
	print("RND:", rnd)
	if BotApi.Instance.army == "axis" then
		if rnd > 0.7 then
			bot_army_local = "ita"
		elseif rnd > 0.3 and rnd <= 0.7 then
			bot_army_local = "ger"
		else 
			bot_army_local = "jap"
		end  
	elseif BotApi.Instance.army == "gerita" then
		if rnd > 0.65 then
			bot_army_local = "ita"
		else 
			bot_army_local = "ger"
		end  
	elseif BotApi.Instance.army == "ost" then
		if rnd > 0.65 then
			bot_army_local = "hun"
		else 
			bot_army_local = "ger"
		end  
	elseif BotApi.Instance.army == "allies" then
		if rnd > 0.7 then
			bot_army_local = "uk"
		elseif rnd > 0.3 and rnd <= 0.7 then
			bot_army_local = "rus"
	...
	elseif BotApi.Instance.army == "usuk" then
		if rnd > 0.65 then
			bot_army_local = "uk"
		else 
			bot_army_local = "usaf"
		end 
	elseif BotApi.Instance.army == "frabri" then
		if rnd > 0.55 then
			bot_army_local = "uk"
		else 
			bot_army_local = "fra"
	...
	setVarsInMissionScript(bot_army_local)
    OnGameStartUtility("conquest",bot_army_local)
end
